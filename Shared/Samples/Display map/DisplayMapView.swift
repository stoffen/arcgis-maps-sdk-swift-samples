...
struct DisplayMapView: View {
    ...
    var body: some View {
        ...
        MapView(map: map).task {
            ...
            let tiles = ["http://35.228.152.51/Aldersklasseriskog/MapServer", "http://35.228.152.51/artsdata/MapServer", "http://35.228.152.51/Reguleringsplaner/MapServer", "http://35.228.152.51/mis/MapServer", "http://35.228.152.51/losmasser/MapServer"]
            do {
                for r in rasters {
                ...
                for t in tiles {
                    let tile = ArcGISTiledLayer(url: URL(string: t)!)
                    try await tile.load()
                    map.addOperationalLayer(tile)
                }
            } catch {
                print("\(error)")
            }
